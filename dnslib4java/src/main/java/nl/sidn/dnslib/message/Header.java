...
public class Header {
  ...
  public JsonObject toJSon() {
    return Json.createObjectBuilder().add("id", id).add("qr", qr.getValue())
        .add("opcode", OpCode.getValue())
        .add("flags",
            Json.createObjectBuilder().add("aa", aa).add("tc", tc).add("rd", rd).add("ra", ra)
                .add("ad", ad).add("cd", cd))
        .add("rcode", rcode.name()).add("qdCount", qdCount).add("anCount", anCount)
        .add("nsCount", nsCount).add("arCount", arCount).build();
  }
