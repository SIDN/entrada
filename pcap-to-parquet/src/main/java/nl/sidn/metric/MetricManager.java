...
public class MetricManager {
  ...
  public static String METRIC_IMPORT_UDP_RESPONSE_FRAGMENTED_COUNT =
  ...
  public static String METRIC_IMPORT_TCP_RESPONSE_FRAGMENTED_COUNT =
  ...
  // cache stats
  public static String METRIC_IMPORT_CACHE_EXPPIRED_DNS_QUERY_COUNT =
  ...
  public void flush() {
    if (log.isDebugEnabled()) {
      ...
      for (String key : metricCache.keySet()) {
        log.debug("Metric key: " + key);
        log.debug("Metric value: " + metricCache.get(key));
      }
    }
    ...
    if (graphiteAdapter.isConnected()) {
      for (String key : metricCache.keySet()) {
        ...
        /**
         * Use a threshhold to determine if the value should be sent to graphite low values may
         * indicate trailing queries in later pcap files. duplicate timestamps get overwritten by
         * graphite and only the last timestamp value is used by graphite.
         */
        if (m.isUseThreshHold() && m.getValue() < threshhold) {
          log.debug("Metric " + m.getName() + " is below threshold min of " + threshhold
      ...
      log.info("Send metrics: " + buffer.toString());
