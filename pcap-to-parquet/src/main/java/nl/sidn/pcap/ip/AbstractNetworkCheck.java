...
public abstract class AbstractNetworkCheck {
  ...
  public AbstractNetworkCheck(Settings settings) {
    ...
    if (file.exists()) {
      ...
      try {
        ...
        log.debug("Resolver file " + file + " cannot be read, reload from url as backup", e);
        PreInit(file);
      }
    }
  }
    ...
    if (file.exists()) {
      ...
      if (!DateUtils.isSameDay(lastModifiedDate, currentDate)) {
        if (log.isDebugEnabled()) {
          log.debug("Resolver file " + filename + " is too old, updating now.");
        }
        PreInit(file);
      }
    } else {
      if (log.isDebugEnabled()) {
        log.debug("Resolver file " + filename + " does not exist, fetching now.");
      }
      PreInit(file);
    }
  ...
  private void PreInit(File file) {
    ...
    if (subnets.size() > 0) {
      try {
        ...
        log.debug("Resolver file " + file + " cannot be written", e);
      }
    }
  ...
  protected void readFromFile(File file) throws IOException {
    ...
    while ((sCurrentLine = br.readLine()) != null) {
      try {
        if (log.isDebugEnabled()) {
          log.debug("Read resolver " + sCurrentLine + " from file: " + file.getAbsolutePath());
        }
  ...
  private void writeToFile(File file) throws IOException {
    ...
    for (String range : subnets) {
      if (log.isDebugEnabled()) {
        log.debug("Write resolver " + range + " to file: " + file.getAbsolutePath());
      }
      bw.write(range);
      bw.write(sep);
    }
