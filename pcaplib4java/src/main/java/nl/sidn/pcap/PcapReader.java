...
public class PcapReader implements Iterable<Packet> {
  ...
  private Packet nextPacket() {
    ...
    long packetTimestampMicros = PcapReaderUtil.convertInt(pcapPacketHeader,
    ...
    if (ipProtocolHeaderVersion == 4 || ipProtocolHeaderVersion == 6) {
      ...
      if (PROTOCOL_TCP == packet.getProtocol()) {
        // found tcp protocol
        tcpOrUdpPayload = tcpDecoder.reassemble(packet, packet.getIpHeaderLen(), packetData.length,
            ipStart, packetData);
        /*
        ...
        // found UDP protocol
        tcpOrUdpPayload = udpDecoder.reassemble(packet, packet.getIpHeaderLen(), packetData.length,
            ipStart, packetData);
  ...
  public void setDatagrams(
      Multimap<nl.sidn.pcap.packet.Datagram, nl.sidn.pcap.packet.DatagramPayload> map) {
