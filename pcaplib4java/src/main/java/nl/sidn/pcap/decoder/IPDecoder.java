...
public class IPDecoder {
  ...
  private Multimap<nl.sidn.pcap.packet.Datagram, nl.sidn.pcap.packet.DatagramPayload> datagrams =
      TreeMultimap.create();
  ...
  public byte[] reassemble(Packet packet, byte[] packetData, int ipStart) {
    ...
    if (packet.isFragmented()) {
      ...
      byte[] fragmentPacketData = Arrays.copyOfRange(packetData, ipStart + packet.getIpHeaderLen(),
          ipStart + packet.getTotalLength());
      nl.sidn.pcap.packet.DatagramPayload payload =
          new nl.sidn.pcap.packet.DatagramPayload(packet.getFragOffset(), fragmentPacketData);
      datagrams.put(datagram, payload);

      if (packet.isLastFragment()) {
        Collection<nl.sidn.pcap.packet.DatagramPayload> datagramPayloads =
            datagrams.removeAll(datagram);
        if (datagramPayloads != null && datagramPayloads.size() > 0) {
          ...
          for (nl.sidn.pcap.packet.DatagramPayload datagramPayload : datagramPayloads) {
            ...
            if (prev != null && !datagramPayload.linked(prev)) {
              if (log.isDebugEnabled()) {
                log.debug("Broken datagram chain between " + datagramPayload + " and " + prev
