...
public class IPv6Util {
  ...
  public static long decodeId(byte[] packetData, int ipStart) {
    ...
    if (nxtHdr == IPV6_FRAGMENT_EXTENTION_TYPE) {
      return PcapReaderUtil.convertUnsignedInt(packetData,
          ipStart + IPV6_HEADER_SIZE + IPV6_ID_OFFSET);
    }
  ...
  public static int buildInternetProtocolV6ExtensionHeaderFragment(
      nl.sidn.pcap.packet.Packet packet, byte[] packetData, int ipStart) {
    if (packet.isFragmented()) {
      long id = PcapReaderUtil.convertUnsignedInt(packetData,
          ipStart + IPV6_HEADER_SIZE + IPV6_ID_OFFSET);
      packet.setIpId(id);
